...
// camera modes
enum CameraMode { DEFAULT, NOISE_DEBUG, FIRST_PERSON_SHOOTER, FLY_THROUGH, BEZIER, INFINITE_TERRAIN};
...
void Display(GLFWwindow* window) {
    ...
    if(cam_mode == FLY_THROUGH||FIRST_PERSON_SHOOTER){
        ...
        cam_speed = cam_speed; //* (double)INERTIA;
...
//            direction.addPoint(vec3(0,0,1));
            ...
            path.addPoint(vec3(1.0,1.0,0.4));
            direction.addPoint(vec3(0.3,0.3,0));
    ...
    if(cam_mode != NOISE_DEBUG) {
        water_refl.Bind();
        {
            ...
            grid.Draw(time, model_matrix,  mirror_V_matrix, projection_matrix, false, NOT_DRAW_SAND);
        ...
        grid.Draw(time, model_matrix, view_matrix, projection_matrix, false);
        water.Draw(time, model_matrix,  view_matrix, projection_matrix);
        skybox.Draw(model_matrix, view_matrix, projection_matrix);
    } else {
        view_matrix= view_matrix;
        grid.Draw(time, model_matrix, view_matrix, projection_matrix, true);
    }
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods){
    ...
    case '5' :
        ...
        break;
    case '6' :
        cam_mode = NOISE_DEBUG;
        glfwSetWindowTitle(window, "NOISE DEBUG");
        glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); //cursor disabled
        break;
    }
